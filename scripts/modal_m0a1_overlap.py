"""Modal launcher for the M0A.1 R2/R3 overlap and structural saturation audit.

CPU only, zero GPU hours authorised. Reuses the frozen candidate pools, the
Package B edge-provenance graphs, and M0A's stored kNN result; writes to its
own output prefix and touches no historical M0A path. Submit through
...
RUNTIME_REPO_ROOT = (
    HOST_REPO_ROOT
    if (HOST_REPO_ROOT / "configs" / "m0a1_overlap.yaml").is_file()
    else Path(REMOTE_ROOT)
)
CONFIG_PATH = RUNTIME_REPO_ROOT / "configs" / "m0a1_overlap.yaml"
CONFIG = yaml.safe_load(CONFIG_PATH.read_text(encoding="utf-8"))
MODAL_CONFIG = CONFIG["modal"]
STORAGE_ROOT = MODAL_CONFIG["storage_root"]

if CONFIG["status"] != "DECLARED":
    raise RuntimeError("M0A.1 is not declared; no declaration, no launch")
if MODAL_CONFIG.get("gpu") is not None:
    raise RuntimeError("M0A.1 authorises zero GPU hours")
...
image = (
    ...
    .add_local_file(str(CONFIG_PATH), remote_path=f"{REMOTE_ROOT}/configs/m0a1_overlap.yaml")
)
        ...
        jobs.append(
            {
                ...
                "m0a_probe_remote": str(
                    PurePosixPath(STORAGE_ROOT)
                    / "outputs"
                    / "m0a_probe"
                    / dataset
                    / fingerprint[:16]
                    / MODAL_CONFIG["source_m0a_execution_label"]
                    / "probe.json"
                ),
            }
        )
...
def _runner_args(job: dict[str, Any]) -> argparse.Namespace:
    held_fixed = CONFIG["budget_curve"]["held_fixed"]
    output_root = (
        PurePosixPath(STORAGE_ROOT)
        / "outputs"
        / "m0a1_overlap"
    ...
    return argparse.Namespace(
        ...
        per_seed_cap=int(held_fixed["per_seed_cap"]),
        neighbour_scan_cap_per_seed=int(held_fixed["neighbour_scan_cap_per_seed"]),
        edge_provenance_root=Path(job["graph_root"]),
        edge_families=list(CONFIG["edge_provenance_control"]["families"]),
        m0a_probe=Path(job["m0a_probe_remote"]),
        output=Path(output_root) / "overlap.json",
    )
...
def run_overlap(job: dict[str, Any]) -> dict[str, Any]:
    os.chdir(REMOTE_ROOT)
    from scripts.run_m0a1_overlap import run
...
def main(datasets: str = "squad_clean,2wiki_clean,metaqa") -> None:
    ...
    if unknown:
        raise ValueError(f"Undeclared M0A.1 datasets: {sorted(unknown)}")
    jobs = _jobs(requested)
    results = list(
        run_overlap.map(jobs, return_exceptions=True, wrap_returned_exceptions=False)
    )
    failures = [item for item in results if isinstance(item, BaseException)]
    if failures:
        raise RuntimeError(f"{len(failures)} M0A.1 overlap job(s) failed: {failures}")
    local_root = HOST_REPO_ROOT / "outputs" / "m0a1_overlap"
