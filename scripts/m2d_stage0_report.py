...
def next_matrix(declared: dict[str, Any]) -> list[str]:
    ...
    lines = [
        ...
        "`stage_1_authorised` is still false, and section 20 puts the review here."),
        "",
...
def render(
    ...
    lines = [
        "# M2D Stage 0 — report",
        "",
        (f"Status: `{declared['status']}`. This document is generated by "
    ...
    lines += [
        ...
        "## STOP_FOR_REVIEW",
        "",
        ("Section 20's sequence is report, verdict, stop. All three are above. "
        "`stage_1_authorised` remains false in the declaration, and the next action is a "
        "human decision on the reading of arm A3's precondition, not a launch."),
        "",
    ]
    return "\n".join(lines)
