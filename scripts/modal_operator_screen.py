"""Modal A10G runner for the gated complete-data operator screen."""
...
RUNTIME_REPO_ROOT = (
    HOST_REPO_ROOT
    if (HOST_REPO_ROOT / "configs" / "operator_screen.yaml").exists()
    else Path(REMOTE_ROOT)
)
CONFIG_PATH = RUNTIME_REPO_ROOT / "configs" / "operator_screen.yaml"
...
image = (
    ...
    .add_local_file(
        str(CONFIG_PATH), remote_path=f"{REMOTE_ROOT}/configs/operator_screen.yaml"
    )
)
...
def _runner_args(job: dict[str, Any]) -> list[str]:
    ...
    args = [
        ...
        *CONFIG["models"],
        ...
        str(screen["epochs"]),
        "--batch-size",
        str(screen["batch_size"]),
        "--hidden-dim",
        str(screen["hidden_dim"]),
        "--layers",
        str(screen["layers"]),
        "--offset-directions",
        str(screen["offset_directions"]),
        "--dropout",
        str(screen["dropout"]),
        "--temperature",
        str(screen["temperature"]),
        "--learning-rate",
        str(screen["learning_rate"]),
        "--weight-decay",
        str(screen["weight_decay"]),
        "--ks",
        *[str(k) for k in screen["ks"]],
        "--device",
        "cuda",
        "--nodes-sha256",
        job["file_sha256"]["nodes.npy"],
    ]
    return args
...
def run_dataset(job: dict[str, Any]) -> dict[str, Any]:
    ...
    process = subprocess.run(
        [sys.executable, "scripts/run_operator_screen.py", *_runner_args(job)],
        check=False,
        text=True,
    ...
    if process.returncode:
        raise RuntimeError(
            f"operator screen failed for {job['dataset']} with {process.returncode}"
        )
    volume.commit()
    manifest = {
        "status": CONFIG["status"],
    ...
    print(json.dumps({"event": "operator_screen_complete", **manifest}), flush=True)
    return manifest
...
def main(datasets: str = "webqsp,2wiki_clean,musique_clean") -> None:
    requested = datasets.split(",")
    unknown = set(requested) - set(CONFIG["datasets"])
    if unknown:
        raise ValueError(f"Unregistered operator-screen datasets: {sorted(unknown)}")
    source_root = Path(
        os.environ.get(
    ...
    for dataset in requested:
        source = source_root / CONFIG["datasets"][dataset]["source_subdir"]
        ...
        result_remote = (
            f"{STORAGE_ROOT}/outputs/operator_screen/{dataset}/{fingerprint}/result.json"
        )
        jobs.append(
            {
                "dataset": dataset,
    ...
    if failures:
        raise RuntimeError(f"{len(failures)} operator screen(s) failed: {failures}")
    local_root = HOST_REPO_ROOT / "outputs" / "operator_screen"
