"""Modal launcher for the frozen validation-only phase screen."""
...
CONFIG_PATH = RUNTIME_REPO_ROOT / "configs" / "phase_screen.yaml"
...
image = (
    ...
    .add_local_file(str(CONFIG_PATH), remote_path=f"{REMOTE_ROOT}/configs/phase_screen.yaml")
    .add_local_file(
...
def _runner_args(job: dict[str, Any]) -> argparse.Namespace:
    ...
    confirmation_config = job["confirmation"]["config"]
    ...
    return argparse.Namespace(
        ...
        axis=job["axis"],
        rate=float(job["rate"]),
        perturbation_seed=int(CONFIG["perturbation_seeds"][job["axis"]]),
        ...
        clean_topology_cache=Path(confirmation_config["topology_cache"]),
        clean_feature_cache=Path(confirmation_config["feature_cache"]),
...
def run_regime(job: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "axis": job["axis"],
        "rate": job["rate"],
        "validation_delta_R@5": result["validation_gnn_minus_qls"]["recall@5"],
...
def main(
    ...
    if unknown:
        raise ValueError(f"Unregistered phase-screen datasets: {sorted(unknown)}")
    jobs = _jobs(requested)
    results = list(run_regime.map(jobs, return_exceptions=True, wrap_returned_exceptions=False))
    failures = [item for item in results if isinstance(item, BaseException)]
    if failures:
        raise RuntimeError(f"{len(failures)} phase-screen job(s) failed: {failures}")
    local_root = HOST_REPO_ROOT / "outputs" / "phase_screen"
    for result in results:
        path = (
            local_root
            / result["dataset"]
            / result["axis"]
            / f"rate_{_rate_key(result['rate'])}.json"
        )
        _download(result["output_remote"], path)
