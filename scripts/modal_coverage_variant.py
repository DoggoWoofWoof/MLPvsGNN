"""Modal A10G runner for the one preregistered set-coverage variant."""
...
RUNTIME_REPO_ROOT = (
    HOST_REPO_ROOT
    if (HOST_REPO_ROOT / "configs" / "coverage_variant.yaml").exists()
    else Path(REMOTE_ROOT)
)
CONFIG_PATH = RUNTIME_REPO_ROOT / "configs" / "coverage_variant.yaml"
...
image = (
    ...
    .add_local_file(
        str(CONFIG_PATH), remote_path=f"{REMOTE_ROOT}/configs/coverage_variant.yaml"
    )
)
...
def _runner_args(job: dict[str, Any]) -> list[str]:
    ...
    return [
        ...
        "--directions", str(model["directions"]),
        "--expected-parameters", str(model["parameters_expected"]),
        ...
        "--diversity-weight", str(objective["diversity"]["weight"]),
        "--diversity-cosine-margin", str(objective["diversity"]["cosine_margin"]),
...
def run_dataset(job: dict[str, Any]) -> dict[str, Any]:
    ...
    process = subprocess.run(
        [sys.executable, "scripts/run_coverage_variant.py", *_runner_args(job)],
        check=False,
        text=True,
    ...
    if process.returncode:
        raise RuntimeError(f"coverage variant failed for {job['dataset']} with {process.returncode}")
    volume.commit()
    manifest = {
        "status": CONFIG["status"],
        "dataset": job["dataset"],
    ...
    print(json.dumps({"event": "coverage_variant_complete", **manifest}), flush=True)
    return manifest
...
def main(datasets: str = "2wiki_clean,musique_clean") -> None:
    ...
    if unknown:
        raise ValueError(f"Unregistered coverage-variant datasets: {sorted(unknown)}")
    source_root = Path(
        os.environ.get(
    ...
    for dataset in requested:
        ...
        result_remote = (
            f"{STORAGE_ROOT}/outputs/coverage_variant/{dataset}/{fingerprint}/result.json"
        )
        jobs.append(
            {
                "dataset": dataset,
    ...
    if failures:
        raise RuntimeError(f"{len(failures)} coverage-variant job(s) failed: {failures}")
    local_root = HOST_REPO_ROOT / "outputs" / "coverage_variant"
